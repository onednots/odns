...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("MYM")
    updateDisplayUnit();
}
...
QString QRCodeDialog::getURI()
{
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non MYM unit input in lnReqAmount, we generate the URI with MYM as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(IcebergcoinUnits::format(IcebergcoinUnits::MYM, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
